...
pub struct ThreadManager {
    ...
    /// 就绪队列（现在我们将用它来模拟 EDF 的就绪队列，每次 fetch 时找出 deadline 最小的）
...
impl ThreadManager {
    /// 创建空的线程管理器
    pub fn new() -> Self {
        Self { tasks: BTreeMap::new(), ready_queue: Vec::new() }
    }
}
    ...
    fn fetch(&mut self) -> Option<ThreadId> {
        ...
        for (i, &tid) in self.ready_queue.iter().enumerate() {
            if let Some(thread) = self.tasks.get(&tid) {
                // 如果启用了 PI，这里可以根据动态优先级辅助调度
                // 但 EDF 主要是看 deadline
                if thread.deadline < min_deadline {
                    min_deadline = thread.deadline;
                    min_idx = i;
                } else if thread.deadline == min_deadline {
                    // 如果 deadline 相同，优先调度动态优先级高（值越小）的
                    if let Some(min_thread) = self.tasks.get(&self.ready_queue[min_idx]) {
                        if thread.dynamic_priority < min_thread.dynamic_priority {
                            min_idx = i;
                        }
                    }
                }
