...
mod impls {
    ...
    impl SyncMutex for SyscallContext {
        ...
        fn mutex_unlock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            // Priority Inheritance 逻辑：释放锁时恢复原优先级
            let current_tid = unsafe { (*processor).current().unwrap().tid };
            unsafe {
                if let Some(current_thread) = (*processor).thread_manager.get_mut(current_tid) {
                    current_thread.dynamic_priority = current_thread.base_priority;
                }
            }
        ...
        fn mutex_lock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            let current = unsafe { (*processor).current().unwrap() };
            let tid = current.tid;
            let current_prio = current.dynamic_priority;
            let current_proc = unsafe { (*processor).get_current_proc().unwrap() };
            ...
            // Priority Inheritance 逻辑
            // 如果锁已经被其他线程持有，且该持有者的优先级低于当前请求线程的优先级（值越小优先级越高）
            if let Some(owner_tid) = current_proc.mutex_owners[mutex_id] {
                // TODO: 假设存在全局可访问的方法来提升 owner 的优先级，这里简化为：
                // 如果能获取到 owner_tid 的任务控制块，则更新其 dynamic_priority = min(dynamic_priority, current_prio)
                unsafe {
                    if let Some(owner_thread) = (*processor).thread_manager.get_mut(owner_tid) {
                        if current_prio < owner_thread.dynamic_priority {
                            owner_thread.dynamic_priority = current_prio;
                        }
                    }
                }
            }
